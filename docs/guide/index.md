...
Welcome to Node Packages, a focused collection of TypeScript packages used across Vue, browser, Bun, Cloudflare Workers, and WebKit-native projects. Each package is published independently and documented with the same structure: installation, usage examples, parameters, returns, type signatures, and internal public dependencies where relevant.
...
- [HTTP Core](/http-core/) &mdash; runtime-agnostic HTTP method and status vocabulary, the shared response class, query-string helpers, and the error classes the server packages throw.
...
- [Cloudflare Worker](/cloudflare-worker/) &mdash; typed routing, scheduled handlers, worker setup, and response helpers for the Cloudflare Workers runtime.
